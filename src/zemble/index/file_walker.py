...
def _load_ignore_for_dir(directory: Path) -> GitIgnoreSpec | None:
    """Loads a gitignore and zembleignore for a dir."""
    gitignore = directory / ".gitignore"
    zembleignore = directory / ".zembleignore"

    lines = []
    if gitignore.is_file():
        lines.extend(gitignore.read_text(encoding="utf-8", errors="ignore").splitlines())
    if zembleignore.is_file():
        lines.extend(zembleignore.read_text(encoding="utf-8", errors="ignore").splitlines())
    if lines:

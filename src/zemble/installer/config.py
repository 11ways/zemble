...
from zemble.installer.agents import ZEMBLE_END, ZEMBLE_PIN, ZEMBLE_START, Action

JsonObjectResult = tuple[Node, bytes] | Literal["skipped", "error"]
_T = TypeVar("_T")

_CODEX_MCP_HEADER = "[mcp_servers.zemble]"
_CODEX_MCP_BLOCK = f'[mcp_servers.zemble]\ncommand = "uvx"\nargs = ["--from", "{ZEMBLE_PIN}", "zemble"]\n'
...
def replace_or_append_marked(path: Path, content: str) -> Action:
    """Replace the marked zemble section in path, or append it if absent."""
    path.parent.mkdir(parents=True, exist_ok=True)
    existed = path.exists()
    existing = path.read_text(encoding="utf-8") if existed else ""

    start_idx = existing.find(ZEMBLE_START)
    end_idx = existing.find(ZEMBLE_END)

    if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
        before = existing[:start_idx]
        after = existing[end_idx + len(ZEMBLE_END) :]
...
def remove_marked(path: Path) -> Action:
    """Remove the marked zemble section from path."""
    if not path.exists():
        return "not-found"
    ...
    after = existing[end_idx + len(ZEMBLE_END) :].lstrip("\n")
...
def _strip_toml_section(text: str, header: str) -> str:
    ...
    prefix = header.strip()[1:-1]  # "[mcp_servers.zemble]" → "mcp_servers.zemble"
...
def merge_toml_block(path: Path) -> Action:
    """Add (or refresh) the zemble [mcp_servers.zemble] table in a Codex config.toml as text."""
    path.parent.mkdir(parents=True, exist_ok=True)
    existed = path.exists()
    existing = path.read_text(encoding="utf-8") if existed else ""
...
def remove_toml_block(path: Path) -> Action:
    """Remove the zemble [mcp_servers.zemble] table from a Codex config.toml, leaving the rest."""
    if not path.exists():
        return "not-found"
    existing = path.read_text(encoding="utf-8")

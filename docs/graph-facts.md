...
| `source ignored by the index` | The source file is inside the workspace but the index's walk skips it - a `build/` segment, a `.gitignore`, a `.zembleignore`. What is left here is generated code no mapper speaks for. | source file |
| `generated (no template)` | The source file is generated Hawkeye Java, and the mapper could not attribute the fact to a template: the class is no template at all (`HawkeyeClassSerializers`), or the fact is not a `call`. | source file |
...
`--json` carries the same list under `skipped`. A Hawkeye workspace used to put
almost everything into the ignored bucket - a javac emitter compiles the generated
`Tpl_*` classes and writes true facts about them, but the index never walked
`build/generated-sources/...`. Those facts are now mapped back onto the `.hwk`
files instead; see **Generated Hawkeye sources** below.
...
coverage: 117 of 118 declared file(s) fresh, 4210 fact edge(s), 309 external target(s), 51 skipped fact(s)
  skipped facts: source outside the index 0, source ignored by the index 44, generated (no template) 0, stale 4, unmapped 3
  generated-source facts mapped to templates: 0 (0 templates)
...
## Generated Hawkeye sources

A Hawkeye workspace compiles every `.hwk` template into a Java class under
`<module>/build/generated-sources/hawkeye/<source set>/java/...`. javac compiles
those classes, so an emitter writes true, precise facts about them - and the index
never walked a `build/` directory, so without a mapping every one of those facts is
dropped. On the javaweb workspace that was **65,857** facts, against 36 that are
generated by something other than Hawkeye.

zemble maps them back onto the template, and the mapped call edges then replace that
template's extracted ones. Nothing here is guessed: every rule below is the Hawkeye
compiler's own, applied forwards.
...
zenit-widget/build/generated-sources/hawkeye/common/java/be/elevenways/hawkeye/
  generated/zenitwidget/Tpl_WidgetDisplayMarkdown.java
  generated/zenitwidget/Tpl_WidgetDisplayMarkdown.sourcemap.json
```
...
Only `CALLS`. A template covered this way keeps its extracted `EXTENDS`, `IMPORTS`
and `REFERENCES_TYPE` edges, because the generated class knows what the compiled
code calls and nothing about what the template extends or renders - a `Tpl_X extends
CompiledTemplate` fact would evict the template's own `{% extend %}`. `override`,
`extends` and `implements` facts about a generated class are therefore counted in
`generated (no template)`, not invented onto the template.

Every mapped edge carries `source = zemble-javac-facts` and keeps the generated
member it was written about, which is what a query then shows:

```
zenit-cms/src/common/templates/shell.hwk:51  called from shell (line 51,
  zemble-javac-facts via be.elevenways.hawkeye.generated.zenitcms.Tpl_Shell#branch14(...))
```
...
1. The **generated** file still hashes to what the facts declare - javac saw what is
   on disk. This is the ordinary `file` line rule.
2. The **template** is not newer than the generated file. Hawkeye records no hash of
   the template in its output, so modification time is the only handle there is. A
   template edited after its class was generated is reported stale with the reason
...
- **A tag class has no line map at all.** Hawkeye emits `// @hwk:` markers only into
  a template class, never into the tag classes a `{% tag %}` declaration generates,
  so a component file's calls land on the tag symbol with no line. In the javaweb
  workspace that is most of `plumage`: 76 templates, 3 sidecars.
- **A template whose class carries no markers** (nothing transpiled to mark) maps by
  class name, also without a line.
- **`HawkeyeClassSerializers`, `HawkeyeCustomElementRegistrations` and
  `HawkeyeDeclaredClassRegistrations`** are generated Hawkeye code that is no
  template. They are counted in `generated (no template)` and stay skipped.
- **A covered template's calls are the compiled template's calls**, which is a
  superset of what the template text writes: the DOM and render-context plumbing the
  transpiler emits is in there too. That is what the compiler did, and it is graded
  as such - but it is not the same list the extractor produced.
- **Generated code from another generator** (`build/generated-sources/blast-autoload`,
  `.../zenit-stylesheets`) is not Hawkeye output and stays in `source ignored by the
  index`.
